...
class _TelaDetalhesConteudoState extends State<TelaDetalhesConteudo> {
  ...
  Future<void> _verificarSeFicheiroExiste() async {
    ...
    final fileName = '${widget.conteudo.id}.${widget.conteudo.tipo}';
  ...
  Future<void> _baixarMaterial() async {
    if (widget.conteudo.urlArquivo.isEmpty) {
      _snack('Este material não possui um ficheiro válido.', erro: true);
      return;
    }
    ...
    if (conexao.contains(ConnectivityResult.none)) {
      _snack('Sem internet para download', erro: true);
      return;
    }
    ...
    try {
      final dir        = await getApplicationDocumentsDirectory();
      final fileName   = '${widget.conteudo.id}.${widget.conteudo.tipo}';
      final caminho    = '${dir.path}/$fileName';
      ...
      // ✅ Guardar download no Supabase
      await _supabase.from('downloads').insert({
        'user_id':       uid,
        'content_id':    widget.conteudo.id.toString(),
        'local_path':    caminho,
        'status':        'completed',
        'downloaded_at': DateTime.now().toIso8601String(),
      });

      _snack('Download concluído!', sucesso: true);
      _verificarSeFicheiroExiste();
    } catch (e) {
      _snack('Erro ao baixar: $e', erro: true);
    } finally {
      if (mounted) setState(() { _estaBaixando = false; _progresso = 0; });
    }
  }

  void _abrirVisualizador() {
    if (_caminhoLocal == null || !_ficheiroExiste) return;
    final tipo = widget.conteudo.tipo.toLowerCase();

    if (tipo.contains('pdf')) {
      Navigator.push(context, MaterialPageRoute(
          builder: (_) => TelaVisualizadorPdf(
              path: _caminhoLocal!, titulo: widget.conteudo.titulo)));
    } else if (tipo.contains('video') || tipo.contains('mp4')) {
      Navigator.push(context, MaterialPageRoute(
          builder: (_) => TelaVisualizadorVideo(
              path: _caminhoLocal!, titulo: widget.conteudo.titulo)));
    } else if (tipo.contains('jpg') || tipo.contains('jpeg') || tipo.contains('png')) {
      Navigator.push(context, MaterialPageRoute(
          builder: (_) => TelaVisualizadorImagem(
              path: _caminhoLocal!, titulo: widget.conteudo.titulo)));
    } else {
      _snack('Formato .$tipo não suportado.');
    }
  }
  ...
  Widget _buildHeader() {
    return Container(
      ...
      child: Center(
        child: Icon(_getIconForType(), size: 80,
  ...
  Widget _buildSubjectChip() {
    return Container(
      ...
      decoration: BoxDecoration(
          color: _chipBg, borderRadius: BorderRadius.circular(20)),
  ...
  Widget _buildInfoSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Sobre este material',
            style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        const Divider(height: 24),
        _infoRow('Disciplina', widget.conteudo.disciplina),
        _infoRow('Tipo de Ficheiro', widget.conteudo.tipo.toUpperCase()),
  ...
  IconData _getIconForType() {
    final tipo = widget.conteudo.tipo.toLowerCase();
    if (tipo.contains('video') || tipo.contains('mp4')) return Icons.play_circle_filled;
    if (tipo.contains('pdf')) return Icons.picture_as_pdf;
    if (tipo.contains('jpg') || tipo.contains('jpeg') || tipo.contains('png')) return Icons.image;
